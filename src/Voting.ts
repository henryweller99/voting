...
import {
  ...
  MinimumBalanceSet as MinimumBalanceSetEvent,
  MinimumTimeSet as MinimumTimeSetEvent,
  Voting as VotingContract,
} from '../generated/templates/Voting/Voting'
import {
  Vote as VoteEntity,
  Cast as CastEntity,
  VotingInfo as VotingInfoEntity,
  VoteCount as VoteCountEntity,
...
export function handleStartVote(event: StartVoteEvent): void {
  ...
  if(vote.metadata.slice(0,5) == 'ipfs:') {
    let description = ipfs.cat(vote.metadata.slice(5))
    //vote.ipfscontent = description
    if(description !== null) {
      let jsonData = json.fromBytes(description as Bytes)
      vote.metadata = jsonData.toObject().get('text').toString()
    }
  }

  let voteCountId = vote.orgAddress.toHex()
  
  let voteCount = VoteCountEntity.load(voteCountId)
  if(!voteCount) {
    voteCount = new VoteCountEntity(voteCountId)
    voteCount.count = 1
  }
  else {
  ...
  vote.castCount = 0
  vote.voteCountSeq = voteCount.count - 1
...
export function handleCastVote(event: CastVoteEvent): void {
  ...
  if(!vote.castCount) {
    ...
    vote.castCount = vote.castCount + 1
  }
...
export function handleMinimumBalanceSet(event: MinimumBalanceSetEvent): void {
  let votingInfo = new VotingInfoEntity(event.transaction.hash.toHex())

  votingInfo.minBalance = event.params.minBalance

  votingInfo.save()
}

export function handleMinimumTimeSet(event: MinimumTimeSetEvent): void {
  let votingInfo = new VotingInfoEntity(event.transaction.hash.toHex())

  votingInfo.minTime = event.params.minTime
...
function _populateVoteDataFromContract(vote: VoteEntity, appAddress: Address, voteNum: BigInt): void {
  ...
  vote.yea = new BigInt(0)
  vote.nay = new BigInt(0)
...
function _populateVoteDataFromEvent(vote: VoteEntity, event: StartVoteEvent): void {
  ...
  if(event.params.minBalance) {
    vote.minBalance = event.params.minBalance
    vote.minTime = event.params.minTime
    vote.totalSupplyAtCreation = event.params.totalSupply
    vote.creatorVotingPower = event.params.creatorVotingPower
  }
}
